...
class TestApp(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_data = {"api_key": "test_key", "model": "test_model", "font_size": 14, "active_tabs": ["Tab 1", "Tab 2"], "user_models": [{"display_name": "Test Model", "full_name": "test_model"}]}
        self.open_patch = patch('builtins.open', unittest.mock.mock_open(read_data=json.dumps(self.config_data)))
    ...
    def test_load_config_creates_default_config_if_not_exists(self):
        ...
        with patch('os.path.exists', return_value=False):
            ...
            # Then
            self.assertEqual(self.app.config, {"api_key": "", "model": "", "font_size": 12, "active_tabs": ["Tab 1"], "user_models": [
    ...
    def test_load_config_loads_existing_config(self):
        # Given
        config_data = {"api_key": "test_key", "model": "test_model", "font_size": 14, "active_tabs": ["Tab 1", "Tab 2"], "user_models": [{"display_name": "Test Model", "full_name": "test_model"}]}
    ...
    def test_save_config_saves_config_to_file(self):
        # Given
        self.app.config = {"api_key": "test_key", "model": "test_model", "font_size": 14, "active_tabs": ["Tab 1", "Tab 2"], "user_models": [{"display_name": "Test Model", "full_name": "test_model"}]}
        ...
        with patch('builtins.open', mock_open):
            ...
            self.assertEqual(json.loads(written_data), {"api_key": "test_key", "model": "test_model", "font_size": 14, "active_tabs": ["Tab 1", "Tab 2", "Tab 3"], "user_models": [{"display_name": "Test Model", "full_name": "test_model"}]})
    ...
    def test_save_current_tab_saves_tab_data_to_file(self):
        ...
        with patch('builtins.open', mock_open):
            ...
            handle = mock_open()
            written_data = "".join(call.args[0] for call in handle.write.call_args_list)
            self.assertEqual(json.loads(written_data), {"tab_name": "Test Tab", "model_name": "test_model", "chat_history": ["message1", "message2"]})

    def test_open_tab_loads_tab_data_from_file(self):
        # Given
        tab_data = {"tab_name": "Test Tab", "model_name": "test_model", "chat_history": ["message1", "message2"]}
        with patch('tkinter.filedialog.askopenfilename', return_value="test_file.json"):
            with patch('builtins.open', unittest.mock.mock_open(read_data=json.dumps(tab_data))):
                self.app.tab_view = MagicMock()
                tab = MagicMock()
                self.app.tab_view.tab.return_value = tab
